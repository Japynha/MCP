    ...
    if command -v docker compose &> /dev/null; then
        ...
        if [ $? -eq 0 ]; then
            ...
            if command -v docker-compose &> /dev/null; then
             ...
             echo -e "${vermelho}   ERRO: Falha ao baixar Docker Compose V1 (${compose_version}).${reset}"
             return 1 # Falha
        fi
    fi
}
...
install_traefik_portainer() {
    ...
    # Loop para garantir que o domínio não seja vazio (VERSÃO CORRIGIDA SEM FORMATAÇÃO HTML)
    ...
    # Loop para garantir que o email não seja vazio (VERSÃO CORRIGIDA SEM FORMATAÇÃO HTML)
    ...
    # Loop para garantir que o token não seja vazio (VERSÃO CORRIGIDA SEM FORMATAÇÃO HTML)
    ...
    # Leitura do Fuso Horário (VERSÃO CORRIGIDA SEM FORMATAÇÃO HTML)
    ...
    # read -p "   - PUID [Padrão: ${PUID}]: " user_puid; PUID=${user_puid:-$PUID}
    # read -p "   - PGID [Padrão: ${PGID}]: " user_pgid; PGID=${user_pgid:-$PGID}
