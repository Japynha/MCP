...
echo \-e "</span>{branco}=> Baixando arquivos de configuração de <span class="math-inline">\{GITHUB\_RAW\_URL\}\.\.\.</span>{reset}"
    ...
    # Loop para garantir que o domínio não seja vazio (VERSÃO CORRIGIDA)
    while [[ -z "$DOMAINNAME" ]]; do
        read -p "   - Digite seu domínio principal (ex: seusite.com): " DOMAINNAME
        if [[ -z "<span class="math-inline">DOMAINNAME" \]\]; then
echo \-e "</span>{vermelho}     O domínio não pode ser vazio!${reset}"
        fi
    done
    # Loop para garantir que o email não seja vazio (VERSÃO CORRIGIDA)
    while [[ -z "$CLOUDFLARE_EMAIL" ]]; do
        read -p "   - Digite seu email da conta Cloudflare: " CLOUDFLARE_EMAIL
         if [[ -z "<span class="math-inline">CLOUDFLARE\_EMAIL" \]\]; then
echo \-e "</span>{vermelho}     O email não pode ser vazio!${reset}"
        fi
    done
    # Loop para garantir que o token não seja vazio (VERSÃO CORRIGIDA)
    while [[ -z "$CLOUDFLARE_DNS_API_TOKEN" ]]; do
        read -p "   - Digite seu Token de API Cloudflare (Permissão: Edit Zone DNS): " CLOUDFLARE_DNS_API_TOKEN
         if [[ -z "<span class="math-inline">CLOUDFLARE\_DNS\_API\_TOKEN" \]\]; then
echo \-e "</span>{vermelho}     O Token API não pode ser vazio!${reset}"
        fi
    done
    # Leitura do Fuso Horário (VERSÃO CORRIGIDA)
    read -p "   - Fuso Horário [Padrão: <span class="math-inline">\{TIMEZONE\}\]\: " user\_timezone
TIMEZONE\=</span>{user_timezone:-$TIMEZONE}

    # As linhas opcionais para PUID/PGID podem ser descomentadas se necessário
    # read -p "   - PUID [Padrão: <span class="math-inline">\{PUID\}\]\: " user\_puid; PUID\=</span>{user_puid:-$PUID}
    # read -p "   - PGID [Padrão: <span class="math-inline">\{PGID\}\]\: " user\_pgid; PGID\=</span>{user_pgid:-<span class="math-inline">PGID\}
